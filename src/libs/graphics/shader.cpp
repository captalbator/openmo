...
void Shader::compile()
{
  ...
  if (!result)
  {
    ...
    if (_path.empty())
      LOG_ERROR("Failed to compile shader with id {}! Error message: {}", _id, log);
    else
      LOG_ERROR("failed to compile shader {} : {}", _path.string(), log);
...
void ShaderProgram::link()
{
  ...
  if (!result)
  {
    ...
    LOG_ERROR("Failed to link program with id {}! Error Message: {}", _id, log);
...
{
  this->setUniform(name, [this, &value](int location) { glUniform2f(location, value.x, value.y); });
}
