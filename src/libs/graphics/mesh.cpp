...
void Mesh::drawInstanced(int count)
{
  ...
  {
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _ibo);
    glDrawElementsInstanced(GL_TRIANGLES, _indices.size(), GL_UNSIGNED_SHORT, &_indices[0], count);
  }
  else
  {
    glDrawArraysInstanced(GL_TRIANGLES, 0, _vertices.size() / (_layout.getStride() / sizeof(float)),
                          count);
  }
}
